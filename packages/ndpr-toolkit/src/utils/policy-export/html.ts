...
/**
 * Returns the URL only if it has an allowed scheme; otherwise returns `''`.
 * Defends against `javascript:`, `data:`, `vbscript:`, etc. landing in an
 * `href` when the policy author submits a malicious website / contact URL.
 *
 * Allowed schemes: `http`, `https`, `mailto`. Scheme-relative (`//foo`) is
 * treated as `https`. Bare hosts without a scheme (`example.com`) are
 * prefixed with `https://` because `<a href="example.com">` resolves to a
 * relative path, which is rarely what a policy author intends.
 */
function safeUrl(raw: string): string {
  const trimmed = raw.trim();
  if (!trimmed) return '';
  // Reject any control characters or whitespace inside the URL — those are
  // commonly used to smuggle disallowed schemes past naive filters.
...
export function exportHTML(policy: PrivacyPolicy, options?: HTMLExportOptions): string {
  ...
  // URL-scheme allowlist defends against `javascript:` / `data:` / `vbscript:`
  // landing in `href`. If the scheme is disallowed, the link is replaced with
  // plain escaped text (still visible, just not clickable).
  const websiteRaw = policy.organizationInfo.website ?? '';
  const safeWebsite = safeUrl(websiteRaw);
  const websiteHtml = websiteRaw
    ? safeWebsite
      ? `<a href="${escapeHtml(safeWebsite)}" target="_blank" rel="noopener">${escapeHtml(websiteRaw)}</a>`
      : escapeHtml(websiteRaw)
    : '';

  const emailRaw = policy.organizationInfo.privacyEmail ?? '';
  // Reject any local-part containing the disallowed schemes' characters; let
  // `safeUrl` validate the assembled mailto: link.
  const safeEmail = emailRaw ? safeUrl(`mailto:${emailRaw}`) : '';
  const emailHtml = emailRaw
    ? safeEmail
      ? `<a href="${escapeHtml(safeEmail)}">${escapeHtml(emailRaw)}</a>`
      : escapeHtml(emailRaw)
    : '';
