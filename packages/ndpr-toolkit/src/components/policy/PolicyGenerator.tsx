...
export const PolicyGenerator: React.FC<PolicyGeneratorProps> = ({
    ...
    return (
      <div className={resolveClass('space-y-4', classNames?.sectionList, unstyled)} role="group" aria-labelledby={`${instanceId}-sections-heading`}>
        ...
        {sections.map(section => {
          const sectionInputId = `${instanceId}-section-${section.id}`;
          const sectionDescId = section.description ? `${instanceId}-section-desc-${section.id}` : undefined;
          return (
              ...
              <div key={sectionId} className="border border-gray-200 dark:border-gray-700 rounded-md p-4">
                ...
                <div className="space-y-4">
                  {sectionVariables.map(variable => {
                    ...
                    const varErrorId = errors[variable.id] ? `${instanceId}-var-err-${variable.id}` : undefined;
                    ...
                    <div key={variable.id}>
                      ...
                      {variable.inputType === 'textarea' ? (
                        <textarea
                          ...
                          aria-invalid={errors[variable.id] ? true : undefined}
                        />
  ...
  const renderPolicyPreview = () => {
    const policyContentId = `${instanceId}-policy-content`;
    return (
      <div>
          ...
          <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-md mb-4" aria-live="polite">
            <div className="prose dark:prose-invert max-w-none">
              {generatedPolicy.split('\n').map((line, index) => {
                if (line.startsWith('## ')) {
                  return <h3 key={index} className="text-xl font-bold mt-6 mb-3">{line.substring(3)}</h3>;
                } else if (line.startsWith('### ')) {
                  return <h4 key={index} className="text-lg font-bold mt-4 mb-2">{line.substring(4)}</h4>;
                } else if (line === '') {
                  return <br key={index} />;
                } else {
